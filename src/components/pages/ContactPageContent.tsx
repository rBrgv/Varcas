'use client'

import { useTranslation } from 'react-i18next'
import { Container, SectionHeading, Card, Button } from '@/components/ui'
import { ScrollReveal } from '@/components/ScrollReveal'
import { SmartEnquiryForm } from '@/components/SmartEnquiryForm'
import '@/lib/i18n'

export function ContactPageContent() {
  const { t } = useTranslation()
  const phoneNumber = '+91 93468 72545'
  const whatsappNumber = '919346872545'
  const email = 'varcasent@gmail.com'

  return (
    <>
      <section className="relative bg-gradient-hero text-white py-24 md:py-32 overflow-hidden">
        <Container className="relative z-10">
          <ScrollReveal variant="fade">
            <h1 className="text-4xl md:text-6xl font-bold mb-6 text-center">
              {t('nav.contact', 'Contact Us')}
            </h1>
            <p className="text-xl text-center text-neutral-200 max-w-3xl mx-auto">
              Get in touch with us for your enterprise solution needs
            </p>
          </ScrollReveal>
        </Container>
      </section>

      <section className="py-20 md:py-24 bg-gradient-elegant relative">
        <div className="absolute inset-0 bg-pattern-mesh opacity-20"></div>
        <Container className="relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <ScrollReveal variant="left">
              <Card>
                <h2 className="text-2xl font-bold text-primary-700 mb-6">Contact Information</h2>
                <address className="not-italic space-y-4">
                  <div>
                    <p className="font-semibold text-primary-700 mb-1">Varcas Enterprises</p>
                    <p className="text-neutral-600">N Divakar Reddy – Proprietor</p>
                  </div>
                  <div>
                    <p className="text-neutral-600">5-5, RGT-SKHT Road</p>
                    <p className="text-neutral-600">Renigunta, Tirupati – 517520</p>
                    <p className="text-neutral-600">Andhra Pradesh, India</p>
                  </div>
                  <div className="space-y-3 pt-4">
                    <a
                      href={`tel:${phoneNumber}`}
                      className="flex items-center gap-3 text-primary-600 hover:text-primary-700 font-medium"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                      </svg>
                      {phoneNumber}
                    </a>
                    <a
                      href={`mailto:${email}`}
                      className="flex items-center gap-3 text-primary-600 hover:text-primary-700 font-medium"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      {email}
                    </a>
                    <a
                      href={`https://wa.me/${whatsappNumber}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-3 text-primary-600 hover:text-primary-700 font-medium"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                      </svg>
                      WhatsApp
                    </a>
                  </div>
                </address>
                <div className="mt-6 space-y-3">
                  <Button
                    variant="primary"
                    as="a"
                    href={`tel:${phoneNumber}`}
                    className="w-full"
                  >
                    Call Now
                  </Button>
                  <Button
                    variant="secondary"
                    as="a"
                    href={`https://wa.me/${whatsappNumber}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full"
                  >
                    WhatsApp
                  </Button>
                  <Button
                    variant="outline"
                    as="a"
                    href={`mailto:${email}`}
                    className="w-full"
                  >
                    Send Email
                  </Button>
                </div>
              </Card>
            </ScrollReveal>
            <ScrollReveal variant="right">
              <Card>
                <h2 className="text-2xl font-bold text-primary-700 mb-6">Find Us</h2>
                <div className="aspect-video bg-neutral-100 rounded-lg overflow-hidden">
                  <iframe
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3870.1234567890123!2d79.3456789!3d13.6322222!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zMTPCsDM3JzU2LjAiTiA3OcKwMjAnNDQuNCJF!5e0!3m2!1sen!2sin!4v1234567890123!5m2!1sen!2sin"
                    width="100%"
                    height="100%"
                    style={{ border: 0 }}
                    allowFullScreen
                    loading="lazy"
                    referrerPolicy="no-referrer-when-downgrade"
                    className="w-full h-full"
                  />
                </div>
                <p className="text-sm text-neutral-600 mt-4">
                  <strong>Address:</strong> 5-5, RGT-SKHT Road, Renigunta, Tirupati – 517520, Andhra Pradesh, India
                </p>
              </Card>
            </ScrollReveal>
          </div>
        </Container>
      </section>

      <section className="py-20 md:py-24 bg-gradient-warm relative">
        <div className="absolute inset-0 bg-pattern-mesh opacity-15"></div>
        <Container className="relative z-10">
          <ScrollReveal variant="fade">
            <SectionHeading className="text-center mb-12">
              Send Us a Message
            </SectionHeading>
            <div className="max-w-2xl mx-auto">
              <SmartEnquiryForm />
            </div>
          </ScrollReveal>
        </Container>
      </section>
    </>
  )
}

